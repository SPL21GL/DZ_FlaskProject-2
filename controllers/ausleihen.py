...
def del_ausleihen_base():
    ...
    if delete_Ausleihen.validate_on_submit():
        ...
        AusleihenID_to_delete = delete_Ausleihen.AusleihenID.data
        Ausleihen_to_delete = db.session.query(Ausleihen).filter(
            Ausleihen.AusleihenID == AusleihenID_to_delete)
        ...
        print("Fatal Error")
    flash(f"Ausleihen with id {AusleihenID_to_delete} has been deleted")
    return redirect("/ausleihen")
